...
CoList::CoList(CoList&& obj)
{
    swap(_list, obj._list);
    swap(_map_list_iter, obj._map_list_iter);
}

CoList& CoList::operator=(CoList&& obj)
{
    swap(_list, obj._list);
    swap(_map_list_iter, obj._map_list_iter);
    return *this;
...
void CoList::push_front(std::shared_ptr<Coroutine> node)
{
    _list.push_front(node);
    _map_list_iter[node] = _list.begin();
}

void CoList::push_back(std::shared_ptr<Coroutine> node)
{
    _list.push_back(node);
    _map_list_iter[node] = --_list.end();
}